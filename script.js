...
// ========= פונקציות מצב "מי מכיר את מי" =========
let whoKnowsQuestions = [];
let qIndex = 0;
let qScores = [0, 0];
let answeredQuestions = [];

function launchWhoKnows() {
  const name1 = document.getElementById("player1Name").value || "שחקן 1";
  const name2 = document.getElementById("player2Name").value || "שחקן 2";
  qScores = [0, 0];
  qIndex = 0;
  answeredQuestions = [];
  ...
  const numQuestions = parseInt(document.getElementById("numCards").value) || 10;

  fetch("cardsWhoKnows.js")
    .then((res) => res.text())
    .then((text) => {
      eval(text); // יוצר whoKnowsCards
      whoKnowsQuestions = shuffle(whoKnowsCards).slice(0, numQuestions);
      showWhoKnowsQuestion();
    });
}

function showWhoKnowsQuestion() {
  const card = whoKnowsQuestions[qIndex];
  const area = document.getElementById("questionArea");
  const name1 = document.getElementById("player1Name").value || "שחקן 1";
  const name2 = document.getElementById("player2Name").value || "שחקן 2";

  if (!card) {
    let summary = `סיום המשחק!\n\n${name1}: ${qScores[0]} נקודות\n${name2}: ${qScores[1]} נקודות\n`;
    if (qScores[0] > qScores[1]) summary += `\n🥇 המנצח: ${name1}`;
    else if (qScores[1] > qScores[0]) summary += `\n🥇 המנצח: ${name2}`;
    else summary += `\n🤝 תיקו!`

    area.innerHTML = `<pre>${summary}</pre><br><button onclick="copySummaryToClipboard()">📋 העתק סיכום</button>`;
...
function answerQuestion(player) {
  if (player === 1 || player === 2) qScores[player - 1]++;
  answeredQuestions.push({
    text: whoKnowsQuestions[qIndex].text,
    correct: player
  });
  nextQuestion();
}
...
function updateWhoKnowsScore() {
  const name1 = document.getElementById("player1Name").value || "שחקן 1";
  const name2 = document.getElementById("player2Name").value || "שחקן 2";
  document.getElementById("whoKnowsScore").innerText = `${name1}: ${qScores[0]} | ${name2}: ${qScores[1]}`;
}

function copySummaryToClipboard() {
  const name1 = document.getElementById("player1Name").value || "שחקן 1";
  const name2 = document.getElementById("player2Name").value || "שחקן 2";
  let summary = `סיכום שאלות "מי מכיר את מי":\n`;
  answeredQuestions.forEach((q, i) => {
    const correct = q.correct === 1 ? name1 : name2;
    summary += `${i + 1}. ${q.text} → ${correct} צדק\n`;
  });
  summary += `\n${name1}: ${qScores[0]} נקודות\n${name2}: ${qScores[1]} נקודות`;
  navigator.clipboard.writeText(summary).then(() => alert("📋 הסיכום הועתק!"));
}